...
/* =========================================================
   V7.3 — scroll-synced editorial work stage
========================================================= */
(()=>{
  const root=document.querySelector('[data-motion-work]');
  if(!root)return;
  const steps=[...root.querySelectorAll('[data-work-step]')];
  const stage=root.querySelector('.motion-work-stage');
  const media=root.querySelector('.motion-work-media');
  const imgA=root.querySelector('[data-work-img-a]');
  const imgB=root.querySelector('[data-work-img-b]');
  const title=root.querySelector('[data-work-title]');
  const type=root.querySelector('[data-work-type]');
  const subtitle=root.querySelector('[data-work-subtitle]');
  const current=root.querySelector('[data-work-current]');
  const link=root.querySelector('[data-work-link]');
  const linkText=root.querySelector('[data-work-link-text]');
  const nav=[...root.querySelectorAll('[data-work-jump]')];
  const reduce=matchMedia('(prefers-reduced-motion: reduce)').matches;
  const mobile=matchMedia('(max-width: 760px)');
  let active=0, front=imgA, back=imgB, imageIndex=0, rotateTimer=null, locked=false;

  const dataOf=i=>{
    const el=steps[i];
    return {title:el?.dataset.title||'',type:el?.dataset.type||'',subtitle:el?.dataset.subtitle||'',href:el?.dataset.href||'#',ratio:el?.dataset.ratio||'4/5',images:(el?.dataset.images||'').split('|').filter(Boolean)};
  };

  function swapImage(src,instant=false){
    if(!src||!front||!back)return;
    if(instant){front.src=src;back.src=src;front.classList.add('is-front');back.classList.remove('is-front');return;}
    back.src=src;
    const reveal=()=>{
      media?.classList.add('is-changing');stage?.classList.add('is-changing');
      requestAnimationFrame(()=>{back.classList.add('is-front');front.classList.remove('is-front')});
      setTimeout(()=>{const old=front;front=back;back=old;media?.classList.remove('is-changing');stage?.classList.remove('is-changing')},760);
    };
    if(back.complete)requestAnimationFrame(reveal);else back.addEventListener('load',reveal,{once:true});
  }

  function startRotation(){
    clearInterval(rotateTimer);imageIndex=0;
    const d=dataOf(active);if(reduce||d.images.length<2||mobile.matches)return;
    rotateTimer=setInterval(()=>{imageIndex=(imageIndex+1)%d.images.length;swapImage(d.images[imageIndex])},3900);
  }

  function setActive(i,instant=false){
    i=Math.max(0,Math.min(steps.length-1,i));if(i===active&&!instant)return;
    active=i;imageIndex=0;const d=dataOf(i);
    nav.forEach((b,n)=>b.classList.toggle('is-active',n===i));
    if(current)current.textContent=String(i+1).padStart(2,'0');
    if(media)media.style.setProperty('--media-ratio',d.ratio);
    if(link)link.href=d.href;if(linkText)linkText.href=d.href;
    if(!instant){stage?.classList.add('is-changing');setTimeout(()=>stage?.classList.remove('is-changing'),520)}
    setTimeout(()=>{if(title)title.textContent=d.title;if(type)type.textContent=d.type;if(subtitle)subtitle.textContent=d.subtitle},instant?0:150);
    swapImage(d.images[0],instant);startRotation();
  }

  function buildMobile(){
    if(!mobile.matches||stage?.querySelector('.motion-work-mobile-strip'))return;
    clearInterval(rotateTimer);
    const strip=document.createElement('div');strip.className='motion-work-mobile-strip';
    steps.forEach((_,i)=>{const d=dataOf(i),a=document.createElement('a');a.className='motion-mobile-card';a.href=d.href;a.innerHTML=`<div class="motion-mobile-media"><img src="${d.images[0]||''}" alt="${d.title}"></div><small>${String(i+1).padStart(2,'0')} · ${d.type}</small><strong>${d.title}</strong><span>${d.subtitle}</span>`;strip.appendChild(a)});
    stage.innerHTML='';stage.appendChild(strip);
  }

  if(mobile.matches){buildMobile();return;}
  setActive(0,true);

  const io=new IntersectionObserver(entries=>{
    if(locked)return;
    const visible=entries.filter(e=>e.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];
    if(visible){const i=steps.indexOf(visible.target);if(i>=0)setActive(i)}
  },{rootMargin:'-28% 0px -28% 0px',threshold:[.12,.3,.55,.8]});
  steps.forEach(s=>io.observe(s));

  nav.forEach((b,i)=>b.addEventListener('click',()=>{
    locked=true;setActive(i);steps[i]?.scrollIntoView({behavior:reduce?'auto':'smooth',block:'center'});setTimeout(()=>locked=false,900);
  }));

  if(media&&matchMedia('(pointer:fine)').matches){
    media.addEventListener('pointermove',e=>{
      const r=media.getBoundingClientRect(),x=e.clientX-r.left,y=e.clientY-r.top;
      media.style.setProperty('--work-x',`${x/r.width*100}%`);media.style.setProperty('--work-y',`${y/r.height*100}%`);
      media.style.setProperty('--cursor-x',`${x-48}px`);media.style.setProperty('--cursor-y',`${y-48}px`);
      if(!reduce){const rx=((y/r.height)-.5)*-1.4,ry=((x/r.width)-.5)*1.8;media.style.transform=`perspective(1200px) rotateX(${rx}deg) rotateY(${ry}deg)`}
    });
    media.addEventListener('pointerleave',()=>{media.style.transform=''});
  }

  const parallaxEls=[...document.querySelectorAll('.home-case-frame,.home-category-card')];
  if(!reduce&&parallaxEls.length){
    let raf=0;const update=()=>{raf=0;const vh=innerHeight;parallaxEls.forEach((el,i)=>{const r=el.getBoundingClientRect();if(r.bottom<0||r.top>vh)return;const p=((r.top+r.height/2)-vh/2)/vh;el.style.setProperty('--parallax-y',`${Math.max(-12,Math.min(12,p*(i%2?10:-10)))}px`);if(Math.abs(p)<.65)el.classList.add('is-inview')})};
