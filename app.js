...
/* ---------------------------------------------------------
   PAROĞLU MODE — embedded IAM PAROĞLU site
--------------------------------------------------------- */
(() => {
  ...
  const ensureShell = () => {
    ...
    shell.className = 'pm-embedded-mode';
  ...
  const closeMode = () => {
    if (!shell) { setState(false); return; }
    shell.classList.remove('is-open');
    setState(false);
    document.documentElement.classList.remove('pm-mode-open');
    document.body.classList.remove('pm-mode-open');
    setTimeout(() => {
  ...
  toggle.addEventListener('click', () => {
    if (shell && shell.classList.contains('is-open')) closeMode();
    else openMode();
  });

  window.addEventListener('message', e => {
    if (e.origin !== window.location.origin) return;
    if (e.data && e.data.type === 'paroglu-mode-close') closeMode();
  });
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape' && shell && shell.classList.contains('is-open')) closeMode();
  });
