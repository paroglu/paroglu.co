(() => {
  ...
  window.PMData = {
    ...
    async assistant(){ const c=await getClient(); if(!c)return []; const {data}=await c.from('assistant_knowledge').select('*').eq('active',true).order('sort_order',{ascending:true}); return data||[]; },
