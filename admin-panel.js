(() => {
  ...
  let cache={projects:[],brands:[],briefs:[],content:[],media:[]};
  ...
  async function loadAll(){
    ...
    const [p,b,br,c,m]=await Promise.all([
    ...
    const err=p.error||b.error||br.error||c.error||m.error;if(err){console.error(err);alert('Panel verileri yüklenemedi. V3 SQL migration çalıştırıldı mı?\n\n'+err.message);return}
    cache={projects:p.data||[],brands:b.data||[],briefs:br.data||[],content:c.data||[],media:m.data||[]};
    metrics();renderProjects();renderBrands();renderBriefs();renderContent();renderMedia();
  }
  ...
  $('#mediaForm')?.addEventListener('submit',async e=>{
    e.preventDefault();const form=e.currentTarget,status=$('#mediaUploadStatus');const fd=new FormData(form);const file=fd.get('file');if(!(file instanceof File)||!file.size)return;
    try{status.textContent='Orijinal dosya yükleniyor…';const up=await uploadOriginal(file,'library');const type=file.type.startsWith('video/')?'video':'image';const {error}=await client().from('media_library').insert({title:String(fd.get('title')||''),file_url:up.url,storage_path:up.path,media_type:type,ratio:String(fd.get('ratio')||'original'),alt_text:String(fd.get('alt_text')||''),file_name:file.name,file_size:file.size});if(error)throw error;status.textContent='Yüklendi ✓';form.reset();await loadAll()}catch(err){status.textContent='Hata: '+err.message}
  });
